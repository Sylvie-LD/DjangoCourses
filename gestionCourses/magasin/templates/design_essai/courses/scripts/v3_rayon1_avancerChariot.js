...
const initCanvas = (id) => {
  ...
  fabric.Image.fromURL('chariot.png', function(img) {
    ...
    canvas.on('mouse:down', function(event) {
      ...
      img.animate('left', targetX, {
        ...
        onComplete: () => { // Une fois le mouvement horizontal terminé
          // Animer le déplacement vertical
          img.animate('top', targetY, {
            duration: 5000, // Durée de l'animation en millisecondes
            onChange: canvas.renderAll.bind(canvas), // Redessiner le canvas à chaque étape de l'animation
            onComplete: () => {
               // Vérifier si le chariot est sur le rayon 1
               if (isChariotOnRayon1(img)) {
                // Afficher la liste de courses sous forme de alert()
                alert("Produits à prendre dans le Rayon 1 :\n- " + produitsRayon1.join("\n- "));
              }
            }
          });
        }
  ...
  // Fonction pour vérifier si le chariot est sur le Rayon 1
  function isChariotOnRayon1(img) {
    ...
    if (rayon1) {
      const rayon1BoundingRect = rayon1.getBoundingRect();
      const chariotCenterX = img.left + (img.width * img.scaleX) / 2;
      const chariotCenterY = img.top + (img.height * img.scaleY) / 2;
      
      return (
        chariotCenterX >= rayon1BoundingRect.left &&
        chariotCenterX <= rayon1BoundingRect.left + rayon1BoundingRect.width &&
        chariotCenterY >= rayon1BoundingRect.top &&
        chariotCenterY <= rayon1BoundingRect.top + rayon1BoundingRect.height
      );
    }
    return false;
  }
